...
ParameterStore::ParameterStore(const std::string dbFile) : SQLiteDB(dbFile) {
	populate_db_schema();
}

void ParameterStore::removeNonOptimalParamSets(const std::string &ticker, const std::string &algoName) {
...
void ParameterStore::assignDateTimesToParamSet(const std::string &ticker, const std::string &algoName, const DateTime &dt_start, const DateTime &dt_end) {
	int algo_inst_id = get_algo_inst_id(ticker, algoName);
...
void ParameterStore::assignDefaultParamSet(const std::string &ticker, const std::string &algoName, const Variables &vars, const EvaluationResult &result) {
	int algo_inst_id = get_algo_inst_id(ticker, algoName);
...
void ParameterStore::assignOptimalParamSet(const std::string &ticker, const std::string &algoName, const Variables &vars, const EvaluationResult &result) {
	int algo_inst_id = get_algo_inst_id(ticker, algoName);
	int result_id    = newResult(result);
	int paramset_id  = newParamSet(algo_inst_id, result_id);
        newParamValues(paramset_id, vars);
...
double ParameterStore::getDefaultWeight(const std::string &ticker, const std::string &algoName) {
	// each algorithm gets equal weight
	std::string sql = R"(
...
double ParameterStore::getBestWeight(const std::string &ticker, const std::string &algoName) {
	std::string sql1 = R"(
		SELECT SUM(Result.totalPercentGain) FROM Result
		WHERE Result.id IN
		(
...
Variables ParameterStore::getOptimalVariables(const std::string &ticker, const std::string &algoName) {
...
Variables ParameterStore::getBestVariables(const std::string &ticker, const std::string &algoName) {
	std::vector<Variables> vars = getTopVariables(ticker, algoName);
	return getMedianVars(vars);
}

std::vector<Variables> ParameterStore::getTopVariables(const std::string &ticker, const std::string &algoName) {
...
Variables ParameterStore::getMedianVars(std::vector<Variables> &vars) {
	// get variable names
...
Variables ParameterStore::getResultVariables(int result_id) {
	std::string sql = R"(
...
double ParameterStore::maxResultsColumn(const std::string &ticker, const std::string &algoName, const std::string &columnName) {
	if (!haveAlgoInstance(ticker, algoName)) {
		return -1;
	}
...
bool ParameterStore::haveAlgoInstance(const std::string ticker, const std::string algoName) {
	std::string sql = R"(
...
void ParameterStore::newParamValues(int paramset_id, const Variables &vars) {
...
int ParameterStore::newParamSet(int algo_inst_id, int result_id) {
	std::string sql = "";
...
int ParameterStore::newResult(const EvaluationResult &result) {
	std::string sql = R"(
...
int ParameterStore::newAlgoInstance(const std::string &ticker, const std::string &name, const DateTime &dtFirst, const DateTime &dtLast) {
	std::string sql = "";
...
// private:
void ParameterStore::insert_ignore_ticker(const std::string &ticker) {
	std::string sql = "";
...
int ParameterStore::get_ticker_id(const std::string &ticker) {
...
void ParameterStore::insert_ignore_param_name(const std::string &name) {
	std::string sql = "";
...
int ParameterStore::get_param_name_id(const std::string &name) {
...
void ParameterStore::insert_ignore_algorithm(const std::string &algoName) {
	std::string sql = "";
...
int ParameterStore::get_algorithm_id(const std::string &algoName) {
...
int ParameterStore::get_default_paramset_id(const std::string &ticker, const std::string &algoName) {
	std::string sql = R"(
		SELECT AlgoInstance.default_paramset_id FROM AlgoInstance
		JOIN Ticker ON AlgoInstance.ticker_id = Ticker.id
		JOIN Algorithm ON AlgoInstance.algorithm_id = Algorithm.id
...
int ParameterStore::get_opt_paramset_id(const std::string &ticker, const std::string &algoName) {
	std::string sql = R"(
		SELECT AlgoInstance.opt_paramset_id FROM AlgoInstance
		JOIN Ticker ON AlgoInstance.ticker_id = Ticker.id
		JOIN Algorithm ON AlgoInstance.algorithm_id = Algorithm.id
...
int ParameterStore::get_algo_inst_id(const std::string &ticker, const std::string &algoName) {
	std::string sql = R"(
		SELECT AlgoInstance.id FROM AlgoInstance
		JOIN Ticker ON AlgoInstance.ticker_id = Ticker.id
		JOIN Algorithm ON AlgoInstance.algorithm_id = Algorithm.id
...
void ParameterStore::populate_db_schema() {
	std::string sql = "";
